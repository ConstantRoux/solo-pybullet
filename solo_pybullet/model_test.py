...
if __name__ == "__main__":
    ...
    for i in range(int(duration / dt)):
        ...
        P[3:6] =  foot_trajectory(dt * i, T, x0, y0, z0, H, Lp, y=1)
        P[6:9] =  foot_trajectory(dt * i, T, x0, y0, z0, H, Lp, y=1)
        P[9:12] = foot_trajectory(dt * i + T/2, T, x0, y0, z0, H, Lp, y=1)

        dP = np.zeros((12,))
        dP[0:3] =  d_foot_trajectory(dt * i + T/2, T, x0, y0, z0, H, Lp, y=1)
        dP[3:6] =  d_foot_trajectory(dt * i, T, x0, y0, z0, H, Lp, y=1)
        dP[6:9] =  d_foot_trajectory(dt * i, T, x0, y0, z0, H, Lp, y=1)
        dP[9:12] = d_foot_trajectory(dt * i + T/2, T, x0, y0, z0, H, Lp, y=1)
